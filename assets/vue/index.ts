...
export default createLiveVue({
  ...
  setup: ({ createApp, component, props, slots, plugin, el }) => {
    // Spread `slots` so the host's render reads each slot value and re-renders when an
    // injected (v-inject) slot changes on navigation. Plain `h(component, props, slots)`
    // (the LiveVue default) passes the reactive object but never reads `.default`, so the
    // target slot stays stuck on the SSR-composed initial page after live navigation.
    const app = createApp({ render: () => h(component as Component, props, { ...slots }) })
    app.use(plugin)
    // Nuxt UI plugin — registered per island (this setup runs once per <.vue> mount)
    app.use(ui)
    app.mount(el)
    return app
  },
